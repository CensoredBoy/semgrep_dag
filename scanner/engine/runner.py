"""
AttackRunner -- executes AttackPipelines using PyRIT as the core engine.

PyRIT components used:
- PromptSendingAttack  -- single-turn attacks
- RedTeamingAttack     -- multi-turn with adversarial LLM
- SubStringScorer / SelfAskTrueFalseScorer -- detection
- OpenAIChatTarget     -- plain LLM requests (verify=False)
- ToolEnabledTarget    -- tool_abuse: injects tools into every API request

Tool abuse flow:
  1. ToolEnabledTarget overrides _construct_request_body, adds tools param
  2. Model responds with tool_calls (finish_reason="tool_calls")
  3. PyRIT base class serializes tool_calls as JSON response pieces
  4. SubStringScorer detects tool names/arguments in serialized JSON

AttackResult / AttackRun -- in-memory DTOs only, NOT saved to DB.
"""
...
from scanner.engine.attack_pipeline import AttackPipeline
from scanner.pyrit_adapter.scorer_factory import create_llm_judge_scorer, create_substring_scorer
from scanner.pyrit_adapter.target_factory import create_tool_target
...
def _pyrit_result_to_dto(
    ...
    """Convert a PyRIT AttackResult to our in-memory DTO."""
    ...
    # Try to parse tool_calls from response (PyRIT serializes them as JSON)
    ...
    return AttackResultDTO(
        ...
        converted_prompt=pyrit_result.objective,
...
class AttackRunner:
    """
    Executes a single AttackPipeline using PyRIT.

    For attacks WITH tools (tool_abuse):
      Creates a ToolEnabledTarget that injects tools into every
      OpenAI API request via the tools parameter.

    For attacks WITHOUT tools:
      Uses the standard OpenAIChatTarget.
    """
    ...
    async def run_async(self, pipeline: AttackPipeline) -> AttackRunDTO:
        ...
        try:
            ...
            # If attack has tools -> create ToolEnabledTarget
            # Otherwise -> use standard target
    ...
    def _resolve_target(self, pipeline: AttackPipeline) -> OpenAIChatTarget:
        """
        If attack has tools, create a ToolEnabledTarget that injects them
        into every API request. Otherwise return the standard target.
        """
        if pipeline.has_tools():
            logger.info(
                "Attack %s has %d tools, using ToolEnabledTarget",
                pipeline.name, len(pipeline.tools),
            )
            return create_tool_target(
                config=self.target_config,
                tools=pipeline.get_openai_tools(),
            )
        return self.target

    # ------ PyRIT PromptSendingAttack (single-turn) ------
    ...
    async def _run_prompt_sending(
        ...
        for prompt_dto in pipeline.prompts:
            try:
                ...
                results.append(dto)
                logger.debug("Prompt %s: %s", prompt_dto.id, dto.detector_verdict)
            except Exception as e:
                logger.warning("Prompt %s failed: %s", prompt_dto.id, e)
                results.append(AttackResultDTO(
                    prompt_id=prompt_dto.id,
                    original_prompt=prompt_dto.content,
                    detector_verdict="error",
                    response=f"ERROR: {e}",
                ))

        return results

    # ------ PyRIT RedTeamingAttack (multi-turn) ------
